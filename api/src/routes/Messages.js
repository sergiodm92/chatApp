...
const {
  ...
  createMessage,
  deleteAll,
} = require("../services/messages_services");
const router = Router();
...
router.get("/", async (req, res) => {
  try {
    ...
    return res
      .status(400)
      .json(customResponseError({ message: "Error al traer los mensajes" }));
  }
});
...
router.post("/", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(400)
      .json(customResponseError({ message: "Error al crear el mensaje" }));
  }
});
...
router.delete("/", async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Ha ocurrido un error al eliminar los mensajes" });
  }
});
